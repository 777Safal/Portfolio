...
function Resume() {
  ...
  return (
    <div id='resume' className='w-10/12 mt-10 px-5 py-7 bg-slate-100 mx-auto'>
        ...
        <div className='mt-5'>
           {tabledata.map((val,i)=>{
            return <div key={i} className='lg:grid lg:grid-cols-3 border-b-2 border-amber-400 py-5'>
              <div className='lg:font-normal font-semibold'>{val.h1}</div>
              <div className='lg:text-base text-sm'>{val.yr}</div>
              <div>{val.status}</div>
            </div>
           })}
           <div className='lg:grid lg:grid-cols-3 py-7 border-b-2 border-amber-400'>
            <div><h1 className='lg:font-normal font-semibold'>Skills</h1></div>
            <div></div>
            <ul>
            {skills.map((val,i)=>{
            return <li key={i} className='text-sm list-disc lg:ml-0 ml-5'>
                {val.h1}
            </li>
           })}
            </ul>
